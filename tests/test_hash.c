...
static MunitResult test_empty(const MunitParameter params[], void *data)
{
  ...
  munit_assert_int(hash_find(hash, 2, 3, 5, 37), ==, 37);
  return MUNIT_OK;
}
...
{
  hash_t *hash = make_hash();
  hash_find(hash, 2, 3, 5, 37);
  munit_assert_int(hash_find(hash, 2, 3, 5, 12), ==, 37);
  return MUNIT_OK;
}
...
static MunitResult test_add_two_entries(const MunitParameter params[], void *data)
{
  hash_t *hash = make_hash();
  hash_find(hash, 2, 3, 5, 37);
  hash_find(hash, 1, 2, 4,  2);
  munit_assert_int(hash_find(hash, 2, 3, 5, 14), ==, 37);
  munit_assert_int(hash_find(hash, 1, 2, 4, 15), ==,  2);
  return MUNIT_OK;
}
